...
(() => {
  ...
  function normalize(raw) {
    ...
    for (const k of ['brandName','shortName','legalName','nit','phone','whatsapp','email','address','city','hours','supportText','logoUrl','appIconUrl','paymentQrUrl','nequiNumber','daviplataNumber','bankName','bankAccount']) {
      out[k] = scrubLegacyBrand(out[k]).trim();
    }
  ...
  function textReplace(value) {
    ...
    return String(value).replace(/¡?GO!?\s*HOUSE|GOHOUSE/gi, brand);
  }
  ...
  function brandNode(root) {
    ...
    const scope = root.querySelectorAll ? root.querySelectorAll('[title],[aria-label],[placeholder],[value]') : [];
  ...
  function ensureManifest(kind) {
    ...
    const app = kind === 'driver' ? 'driver' : kind === 'panel' ? 'panel' : 'client';
    link.href = `/api/manifest.webmanifest?app=${app}`;
  }
  ...
  function loadExtensions() {
    ...
    loadExtension('white-label-push-script', '/gohouse-push.js?v=20260919.38');
    if (kind === 'panel') {
      ...
      loadExtension('white-label-driver-codes-script', '/gohouse-driver-codes.js?v=20260918.2');
      loadExtension('white-label-panel-ops-script', '/gohouse-panel-ops.js?v=20260921.43');
      loadExtension('white-label-fleet-panel-script', '/gohouse-fleet-panel.js?v=20260921.43');
    }
    if (kind === 'panel' || kind === 'client') loadExtension('white-label-publicidad-script', '/gohouse-publicidad.js');
    if (kind === 'driver') loadExtension('white-label-fleet-driver-script', '/gohouse-fleet-driver.js?v=20260921.43');
    if (kind === 'client' || kind === 'driver') loadExtension('white-label-install-script', '/gohouse-install.js');
  }

  function enforcePermanentDriverCode() {
    if (pageKind() !== 'panel') return;
    document.querySelectorAll('button[onclick*="cambiarPinRepartidor"],button[title*="PIN nuevo" i],button[title*="cambiar PIN" i]').forEach(btn => btn.remove());
  }
      ...
      observer = new MutationObserver(mutations => {
        for (const mutation of mutations) {
          for (const node of mutation.addedNodes) {
            brandNode(node);
            if (node.nodeType === Node.ELEMENT_NODE) { applyLogo(); enforcePermanentDriverCode(); }
          }
        }
      });
