...
export function pushTemplateVersion() { return 'brand-v31'; }
...
function brandInfo(root) {
  ...
  const rawName = String(cfg.brandName || 'Domicilios').trim() || 'Domicilios';
  const name = /¡?GO!?\s*HOUSE|GOHOUSE/i.test(rawName)
    ? 'Domicilios'
    : rawName;
  return {
    name,
...
export async function notifyStateDiff(before, after) {
  ...
  for (const order of afterOrders) {
    ...
    if (prev.estado !== order.estado && order.clienteUid) {
      const company = brandInfo(after).name;
      await sendToPrincipal('client', order.clienteUid, basePayload(after, {
        title:`${company} · Actualización`,
        body:`Mi domicilio de ${company} #${orderId(order)} va: ${stateLabel(order.estado)}`,
