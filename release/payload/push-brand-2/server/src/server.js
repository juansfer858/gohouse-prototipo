...
app.get('/api/health', async (_req,res) => {
  ...
  res.json({ ok:true, service:'domicilios-llanos-vps', db:true, push:pushReady(), pushTemplate:pushTemplateVersion(), fleet:true, clientRecoveryMail:clientRecoveryMailReady(), state:rows[0] || null });
});
...
app.post('/api/driver-codes/:id/register', authMiddleware(), async (req,res,next) => {
  try {
    ...
    const valid=await loginDriver(driverId,pin);
    if (!valid) return res.status(401).json({error:'INVALID_PIN'});
