...
(() => {
  ...
  async function mount(){
    ...
    const view=$('vista-ajustes'); if(!view||$('gh-recovery-mail-card'))return;
    try{
      const cfg=await readConfig();
      const host=document.createElement('div'); host.innerHTML=cardHtml(cfg); view.appendChild(host.firstElementChild);
  ...
  async function boot(){
    ...
    mount();
    const obs=new MutationObserver(()=>mount());obs.observe(document.body,{childList:true,subtree:true});
