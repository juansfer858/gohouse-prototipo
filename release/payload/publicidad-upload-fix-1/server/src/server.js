...
app.get('/api/health', async (_req,res) => {
  ...
  res.json({ ok:true, service:'domicilios-llanos-vps', db:true, push:pushReady(), pushTemplate:pushTemplateVersion(), driverCodeMode:'admin-set-v33', recoveryMailMode:'gmail-app-password-v35', uploadMode:'safe-web-uploads-v37', fleet:true, clientRecoveryMail:clientRecoveryMailReady(), state:rows[0] || null });
});
...
app.post('/api/upload-data-url', authMiddleware(), async (req,res,next) => {
  try {
    ...
    const uploadDir = String(
      config.uploadDir ||
      process.env.GOHOUSE_UPLOAD_DIR ||
      '/opt/gohouse/web/uploads'
    ).trim();

    if (!uploadDir) {
      const e = new Error('UPLOAD_DIR_NOT_CONFIGURED');
      e.status = 500;
      throw e;
    }
