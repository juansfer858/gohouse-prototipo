...
const PUBLIC_UPLOAD_DIR='/opt/gohouse/web/uploads';
function uploadStorageReady(){
  try{
    fsSync.accessSync('/opt/gohouse/web', fsSync.constants.W_OK);
    return true;
  }catch{
    return false;
  }
...
app.get('/api/health', async (_req,res) => {
  ...
  res.json({ ok:true, service:'domicilios-llanos-vps', db:true, push:pushReady(), pushTemplate:pushTemplateVersion(), driverCodeMode:'admin-set-v33', recoveryMailMode:'gmail-app-password-v35', uploadMode:'postgres-media-v42', uploadStorageReady:true, fleet:true, clientRecoveryMail:clientRecoveryMailReady(), state:rows[0] || null });
});
...
app.post('/api/upload-data-url', authMiddleware(), async (req,res,next) => {
  try {
    ...
    const id = cryptoId();
    await pool.query(
      'INSERT INTO media_assets(id,mime_type,bytes,created_by) VALUES($1,$2,$3,$4)',
      [id,mime,raw,String(req.principal?.uid || req.principal?.email || '')]
    );

    res.status(201).json({url:`/api/public/uploads/${id}`});
  } catch(e){ next(e); }
});

app.get('/api/public/uploads/:id', async (req,res,next) => {
  try {
    const { rows } = await pool.query(
      'SELECT mime_type,bytes FROM media_assets WHERE id=$1 LIMIT 1',
      [String(req.params.id || '')]
    );
    const asset=rows[0];
