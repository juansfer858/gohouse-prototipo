...
(() => {
  ...
  function addStyles(){
    ...
    st.textContent = `
      ...
      .chat-panel,.chat-panel-modal{
        bottom:calc(10px + env(safe-area-inset-bottom) + var(--gh-keyboard-offset,0px))!important;
        height:min(520px,calc(var(--gh-visible-height,100dvh) - 24px))!important;
        max-height:calc(var(--gh-visible-height,100dvh) - 24px)!important;
      }
      ...
      @media(max-width:560px){
        ...
        .chat-panel,.chat-panel-modal{left:8px!important;right:8px!important;width:auto!important;max-width:none!important;margin:0!important;border-radius:14px!important}
        .chat-fab{width:52px!important;height:52px!important}
        .gh-panel-tabs{grid-template-columns:repeat(2,minmax(0,1fr))!important}
        .gh-msg-row{grid-template-columns:42px minmax(0,1fr) auto;padding:10px}
        .gh-msg-avatar{width:38px;height:38px}
      }
  ...
  function bindKeyboardUx(){
    ...
    document.addEventListener('focusin', e => {
      ...
      setTimeout(() => {
        syncVisualViewport();
        if (e.target.closest('.chat-panel,.chat-panel-modal')) e.target.scrollIntoView({block:'nearest'});
      }, 180);
    });
  }
  ...
  async function ensureClientSupportChat(){
    ...
    for(let i=0;i<40;i++){
      const btn=$('chat-soporte-btn');
      if(btn){btn.style.display='flex';btn.setAttribute('aria-label','Chat con la empresa de domicilios');}
      if(typeof window.abrirChatSoporte==='function'){
        try{window.abrirChatSoporte(profile.celular);}catch{}
        return;
      }
      await sleep(150);
    }
  ...
  function patchClientSupportSender(){
    ...
    window.enviarMensajeChatSoporte=async function(){
      const input=$('chat-soporte-input');
      const texto=String(input?.value||'').trim();
      if(!texto)return;
      const profile=clientProfile();
      const key=phoneKey(profile?.celular);
      ...
      input.disabled=true;
      try{
        await window.GoHouseVPS.api('/push-node',{method:'POST',body:JSON.stringify({path:`chatsClientes/${key}`,value:payload})});
        input.value='';
        input.focus({preventScroll:true});
      }catch(err){
        console.error('Error enviando chat de soporte:',err);
        if(typeof window.showToast==='function')window.showToast('No se pudo enviar: '+(err?.message||'error de conexión'));
  ...
  function fmt(ts){const n=Number(ts||0);if(!n)return'';const d=new Date(n),now=new Date();const same=d.toDateString()===now.toDateString();return same?d.toLocaleTimeString('es-CO',{hour:'2-digit',minute:'2-digit'}):d.toLocaleDateString('es-CO',{day:'2-digit',month:'2-digit'});}
  ...
  function convoRows(){
    ...
    for(const [key,node] of Object.entries(panelChat.chats||{})){
      ...
      const messages=Object.values(node).filter(Boolean).sort((a,b)=>(Number(a?.hora)||0)-(Number(b?.hora)||0));
      if(!messages.length)continue;
      const last=messages[messages.length-1]||{};
      const profile=findClientByKey(key);
      const metaMsg=[...messages].reverse().find(m=>m?.clienteNombre||m?.clienteCelular)||{};
      const name=String(metaMsg.clienteNombre||profile?.nombre||'Cliente').trim();
      const phone=String(metaMsg.clienteCelular||profile?.celular||key).trim();
      const city=String(metaMsg.clienteCiudad||profile?.ciudad||'').trim();
      const lastAt=Number(last.hora)||0;
  ...
  function ensurePanelMessagesUI(){
    const main=$('main'),pedidos=$('tab-pedidos');
    if(!main||!pedidos)return false;
    const bar=pedidos.parentElement;bar?.classList.add('gh-panel-tabs');
    let tab=$('tab-mensajes');
    if(!tab){
      tab=document.createElement('button');tab.type='button';tab.className='btn btn-ghost';tab.id='tab-mensajes';tab.innerHTML='💬 Mensajes <span id="gh-msg-badge"></span>';
      const anchor=$('tab-publicidad')||$('tab-ajustes');if(anchor)bar.insertBefore(tab,anchor);else bar.appendChild(tab);
      tab.addEventListener('click',openMessagesView);
    }
  ...
  function closeMessagesView(){$('vista-mensajes')?.classList.remove('is-open');const tab=$('tab-mensajes');if(tab)tab.className='btn btn-ghost';}
  function openMessagesView(){
    if(!ensurePanelMessagesUI())return;
    document.querySelectorAll('#main > [id^="vista-"]').forEach(v=>{if(v.id!=='vista-mensajes')v.style.display='none';});
    const pub=$('vista-publicidad');if(pub)pub.classList.remove('is-open');
    const view=$('vista-mensajes');view.style.display='block';view.classList.add('is-open');
    ['tab-pedidos','tab-informes','tab-usuarios','tab-publicidad','tab-qr','tab-qr-public','tab-ajustes'].forEach(id=>{const el=$(id);if(el)el.className='btn btn-ghost';});
    const tab=$('tab-mensajes');if(tab)tab.className='btn btn-primary';renderPanelMessages();
  }

  function renderPanelMessages(){
    if(!ensurePanelMessagesUI())return;
    const rows=convoRows(),list=$('gh-msg-list'),unreadTotal=rows.reduce((n,r)=>n+(r.unread?1:0),0),badge=$('gh-msg-badge');
    if(badge){badge.textContent=String(unreadTotal);badge.style.display=unreadTotal?'block':'none';}
    if(!list)return;
  ...
  async function initPanelMessages(){
    ...
    for(let i=0;i<60&&(!window.gohouseCloud||!$('tab-pedidos'));i++)await sleep(200);
    if(!window.gohouseCloud)return;
    ensurePanelMessagesUI();
    panelChat.app=window.gohouseCloud.initializeApp({appKind:'panel'},'panel-mensajes-addon');
    const db=panelChat.app.database();panelChat.clientsRef=db.ref('gohouse-data/clientes');panelChat.chatsRef=db.ref('chatsClientes');
    panelChat.clientsRef.on('value',snap=>{panelChat.clients=snap.val()||{};renderPanelMessages();},()=>{});
    panelChat.chatsRef.on('value',snap=>{panelChat.chats=snap.val()||{};renderPanelMessages();},()=>{});
    const mo=new MutationObserver(()=>ensurePanelMessagesUI());const main=$('main');if(main)mo.observe(main,{childList:true,subtree:true});
  }

  async function init(){
    ensureViewport();addStyles();bindKeyboardUx();
    if(kind==='panel')initPanelMessages();
    if(kind==='client'){patchClientSupportSender();ensureClientSupportChat();[800,2000,5000].forEach(ms=>setTimeout(ensureClientSupportChat,ms));}
  }
