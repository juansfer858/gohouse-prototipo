...
(() => {
  ...
  function updatePanelButton(){
    ...
    if(!btn)return;
    btn.textContent=panelButtonLabel();
    btn.dataset.permission=supported()?Notification.permission:'unsupported';
  ...
  async function init(){
    ...
    if(kind()==='panel'){
      ensurePanelButton();
      panelObserver=new MutationObserver(()=>ensurePanelButton());
