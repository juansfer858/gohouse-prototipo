...
(() => {
  ...
  function refreshAddon(tabId){
    ...
    if (tabId === 'tab-publicidad'){
      setTimeout(() => {
        // PanelNav oculta las vistas addon con display:none inline. Al volver
        // a Publicidad hay que retirar ese bloqueo antes de abrir el módulo.
        const ads = $('vista-publicidad');
        if (ads){
          ads.style.removeProperty('display');
          ads.classList.add('is-open');
        }
