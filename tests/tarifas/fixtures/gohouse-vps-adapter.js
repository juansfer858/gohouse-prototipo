...
(() => {
  ...
  class Auth {
    ...
    async _restore(){
      if(!token()){ this._emit(); return; }
      try{
        const out=await api('/auth/session'); this.currentUser=out.user; storeSession(token(),this.currentUser); ensureWs(); this._emit();
      }catch{ this.currentUser=null; storeSession(null,null); this._emit(); }
    }
